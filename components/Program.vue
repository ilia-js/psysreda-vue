<!--NEXT Раздел "Программа для индивидуальной работы"-->
<template>
<div class="col-md-12 s-container">
  <div>
    <h1 class="s-title text-center">Программа индивидуальной работы</h1>
    <h2 class="title-comment">Ориентировочные темы нашей работы -<br/>компас на пути освобождения из со-зависимости</h2>
    <div class="row">
      <div class="col-md-6">
        <div class="block">
          <h1 class="c-title">Обозначаем маршрут</h1>
          <ul class="dash">
            <li>Увидеть и признать реальное положение в отношениях</li>
            <li>Работа с чувствами и потребностями (страх, стыд, вина)</li>
            <li>Умение управлять своими эмоциями</li>
            <li>Инвентаризация жизни, наведение «порядка» во взаимоотношениях</li>
          </ul>
        </div>
      </div>
      <div class="col-md-6">
        <div class="block">
          <h1 class="c-title">Работаем с со-зависимыми моделями</h1>
          <ul class="dash">
            <li>Отрицание</li>
            <li>Низкая самооценка</li>
            <li>Уступчивость</li>
            <li>Контроль</li>
            <li>Избегание</li>
          </ul>
        </div>
      </div>
      <div class="col-md-6">
        <div class="block">
          <h1 class="c-title">Формируем здоровые стратегии</h1>
          <ul class="dash">
            <li>Признание</li>
            <li>Устойчивая самооценка</li>
            <li>Формирование здоровых границ и способность говорить НЕТ</li>
            <li>Доверие, ответственность за свою жизнь</li>
            <li>Открытость, способность разрешать конфликтные ситуации</li>
            <li>Доверительные отношения вместо контроля</li>
            <li>Способность удовлетворять свои потребности</li>
          </ul>
        </div>
      </div>
      <div class="col-md-6">
        <div class="block">
          <h1 class="c-title">Работаем с убеждениями</h1>
          <ul class="dash">
            <li>Работа с ограничивающими убеждениями, родительскими посланиями</li>
            <li>Работа с травмами и разрешение травматического опыта</li>
            <li>
            	Работа с базовым правом
            	<ul class="dash">
            		<li>Быть</li>
            		<li>Право на потребности</li>
            		<li>На свою территорию и ее защиту</li>
        		</ul>
            </li>
            <li>У вас будет поддержка и ресурсы менять свою жизнь</li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</div>
</template>
<style scoped>
  h2{margin-bottom:10px;}
  .block {background: #efefef;padding:20px 15px 5px;margin:20px 30px;border-radius:15px;}
  .s-title{padding:0 20px 20px;}
  .c-title{font-size:1.15rem;font-weight:bold;}
  .title-comment{font-size:1.3rem;font-weight:normal;text-align:center;padding:0 40px 20px;}
  .dash{padding-left:20px;}
</style>